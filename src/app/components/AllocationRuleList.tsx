...
import { Search, Plus, Settings2, RotateCcw, LayoutPanelLeft, CheckCircle, FileText, AlertTriangle, ListFilter, ClipboardCheck } from 'lucide-react';
...
const MOCK_DATA: AllocationRuleRow[] = [
  { id: 1, ruleId: 'RUL-2026-001', allocationName: 'Phân bổ chi phí chung theo doanh thu', allocationType: 'Tỷ lệ doanh thu', effectiveDate: '01/01/2026', expiryDate: '31/12/2026', status: 'active' },
  { id: 2, ruleId: 'RUL-2026-002', allocationName: 'Phân bổ chi phí điện nước theo diện tích', allocationType: 'Tỷ lệ diện tích', effectiveDate: '01/01/2026', expiryDate: '31/12/2026', status: 'active' },
  { id: 3, ruleId: 'RUL-2026-003', allocationName: 'Phân bổ lương gián tiếp theo nhân viên', allocationType: 'Số nhân viên', effectiveDate: '01/02/2026', expiryDate: '31/12/2026', status: 'active' }
];
...
export default function AllocationRuleList({ onCreateClick, onViewClick }: AllocationRuleListProps) {
  ...
  const activeCount = MOCK_DATA.filter(r => r.status === 'active').length;
    ...
    <div className="flex flex-col h-screen bg-gray-50 overflow-hidden text-gray-800">
      ...
      <div className="bg-white border-b px-6 py-2 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          <h1 className="text-sm font-bold text-blue-900 uppercase">Allocation Rules</h1>
          <span className="text-[10px] text-gray-400 font-medium whitespace-nowrap">Quy tắc phân bổ chi phí</span>
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <div className="px-6 py-4 grid grid-cols-3 gap-4">
            <div className="bg-blue-50/40 border border-blue-100 rounded-lg p-3 flex justify-between items-center h-20">
              <div>
                ...
                <p className="text-2xl font-black text-blue-900 leading-none">{MOCK_DATA.length}</p>
              </div>
            ...
            <div className="bg-emerald-50/40 border border-emerald-100 rounded-lg p-3 flex justify-between items-center h-20">
              <div>
                <p className="text-[9px] font-bold text-emerald-600/70 uppercase mb-1">Active Rules</p>
            ...
            <div className="bg-indigo-50/40 border border-indigo-100 rounded-lg p-3 flex justify-between items-center h-20">
              <div>
                <p className="text-[9px] font-bold text-indigo-600/70 uppercase mb-1">Types</p>
                <p className="text-2xl font-black text-indigo-900 leading-none">{new Set(MOCK_DATA.map(r => r.allocationType)).size}</p>
              </div>
          ...
          {selectedRows.length > 0 && (
            <div className="mx-6 mb-2">
              <MasterDataToolbar
                ...
                onSave={() => console.log('Saving rules...')}
            ...
            <table className="w-full text-sm min-w-[1200px]">
              <thead className="sticky top-0 z-30 font-bold uppercase tracking-tight text-[10px]">
                <tr className="bg-[#f0f7ff] border-b">
                  ...
                  <th className="px-3 py-3 text-left text-blue-700 border-r w-32 bg-[#f0f7ff] sticky left-12 z-40 uppercase">Rule ID</th>
                  <th className="px-3 py-3 text-left text-blue-700 border-r w-80 font-medium uppercase">Allocation Name</th>
                  <th className="px-3 py-3 text-left text-blue-700 border-r w-48 font-medium uppercase text-center">Type</th>
                  <th className="px-3 py-3 text-center text-blue-700 border-r w-40 font-medium uppercase">Effective Date</th>
                  <th className="px-3 py-3 text-center text-blue-700 border-r w-40 font-medium uppercase">Expiry Date</th>
              ...
              <tbody>
                {filteredData.map((item) => (
                  <tr key={item.id} className="border-b hover:bg-blue-50/30 transition-colors bg-white">
                    ...
                    <td className="px-3 py-2 border-r bg-white sticky left-12 z-20 text-[11px] font-bold text-blue-600 group cursor-pointer hover:underline flex items-center gap-1.5" onClick={() => onViewClick?.(item.ruleId)}>
                      <FileText className="w-3 h-3 text-blue-400 group-hover:text-blue-600" />
                      {item.ruleId}
                    </td>
                    <td className="px-3 py-2 border-r text-[11px] font-bold text-gray-800">{item.allocationName}</td>
                    <td className="px-3 py-2 border-r text-[11px] text-center font-medium text-blue-600 italic">{item.allocationType}</td>
                    <td className="px-3 py-2 border-r text-[11px] text-center text-gray-500 font-mono tracking-tighter">{item.effectiveDate}</td>
                    <td className="px-3 py-2 border-r text-[11px] text-center text-gray-500 font-mono tracking-tighter">{item.expiryDate}</td>
                    <td className="px-3 py-2 text-center">
                      <span className={`px-2 py-0.5 rounded-[4px] text-[9px] font-bold uppercase tracking-wider ${item.status === 'active' ? 'bg-emerald-100 text-emerald-700 border border-emerald-200' :
          ...
          <div className="bg-white border-t px-6 py-2 flex items-center justify-between shrink-0 font-bold text-[10px] text-gray-500 uppercase tracking-widest text-center">
            <div>Showing <span className="text-blue-600">{filteredData.length}</span> of {MOCK_DATA.length} records</div>
          </div>
        </div>
      </div>
    </div>
