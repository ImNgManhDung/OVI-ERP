...
export default function CostCenterList() {
  ...
  const handleAddRow = () => {
    const newRow: CostCenterRow = {
      id: Date.now(),
      cceId: `CCE-${String(data.length + 1).padStart(3, '0')}`,
      coaId: '',
      lenId: '',
      depOunId: '',
      cceCode: '',
    ...
    <div className="flex flex-col h-screen bg-gray-50 overflow-hidden text-gray-800">
      ...
      <div className="bg-white border-b px-6 py-2 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          <h1 className="text-sm font-bold text-blue-900">Cost Center</h1>
          <span className="text-[10px] text-gray-400 font-medium whitespace-nowrap">Quản lý trung tâm chi phí</span>
        </div>
      </div>
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <div className="px-6 py-4 grid grid-cols-3 gap-4">
            <div className="bg-blue-50/40 border border-blue-100 rounded-lg p-3 flex justify-between items-center h-20">
              <div>
                <p className="text-[9px] font-bold text-blue-600/70 uppercase mb-1">Total Cost Centers</p>
            ...
            <div className="bg-purple-50/40 border border-purple-100 rounded-lg p-3 flex justify-between items-center h-20">
              <div>
                ...
                <p className="text-2xl font-black text-purple-900">{new Set(data.map(r => r.cceType)).size}</p>
              </div>
          ...
          <div className="px-6 pb-2">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-3">
                <Button size="sm" onClick={handleAddRow} className="bg-blue-600 hover:bg-blue-700 text-[11px] font-bold h-8 px-4 shadow-sm gap-2">
            ...
            <table className="w-full text-sm min-w-[2600px]">
              <thead className="sticky top-0 z-10 font-bold uppercase tracking-tight text-[10px]">
                <tr className="bg-[#f0f7ff] border-b">
                  ...
                  <th className="px-3 py-3 text-left text-blue-700 border-r w-32 bg-[#f0f7ff] sticky left-12 z-20">Cost Center ID</th>
                  <th className="px-3 py-3 text-left text-blue-700 border-r w-48">Chart of Account ID</th>
                  <th className="px-3 py-3 text-left text-blue-700 border-r w-48">Legal Entity ID</th>
                  <th className="px-3 py-3 text-left text-blue-700 border-r w-48">Dept Unit ID</th>
                  <th className="px-3 py-3 text-left text-blue-700 border-r w-48">Cost Center Code</th>
                  <th className="px-3 py-3 text-left text-blue-700 border-r w-64">Cost Center Name</th>
                  <th className="px-3 py-3 text-left text-blue-700 border-r w-80">Description</th>
                  <th className="px-3 py-3 text-left text-blue-700 border-r w-48">Type</th>
                  <th className="px-3 py-3 text-left text-blue-700 border-r w-48">Class</th>
                  <th className="px-3 py-3 text-left text-blue-700 border-r w-32">Status</th>
                  <th className="px-3 py-3 text-left text-blue-700 border-r w-40">From Date</th>
                  <th className="px-3 py-3 text-left text-blue-700 border-r w-40">To Date</th>
                  <th className="px-3 py-3 text-left text-blue-700 border-r w-32">Created By</th>
                  <th className="px-3 py-3 text-left text-blue-700 w-32">Create Date</th>
                </tr>
              </thead>
              <tbody>
                {filteredData.map((row) => (
                  <tr key={row.id} className="border-b hover:bg-blue-50/30 transition-colors bg-white">
                    ...
                    <td className="px-3 py-2 border-r bg-white sticky left-12 z-10 text-[11px] font-bold text-blue-600">{row.cceId}</td>
                    ...
                    <td className="px-3 py-2 border-r font-bold text-[11px]"><Input value={row.cceCode} onChange={(e) => updateRow(row.id, 'cceCode', e.target.value)} className="h-7 border-transparent hover:border-gray-200 shadow-none text-[11px]" /></td>
                    <td className="px-3 py-2 border-r font-medium text-[11px]"><Input value={row.cceName} onChange={(e) => updateRow(row.id, 'cceName', e.target.value)} className="h-7 border-transparent hover:border-gray-200 shadow-none text-[11px]" /></td>
                    <td className="px-3 py-2 border-r text-[11px]"><Input value={row.cceDescription} onChange={(e) => updateRow(row.id, 'cceDescription', e.target.value)} className="h-7 border-transparent hover:border-gray-200 shadow-none text-[11px]" /></td>
                    <td className="px-3 py-2 border-r text-[11px]">
                      <Select value={row.cceType} onValueChange={(v) => updateRow(row.id, 'cceType', v)}>
                    ...
                    <td className="px-3 py-2 border-r text-[11px]">
                      <Select value={row.cceClass} onValueChange={(v) => updateRow(row.id, 'cceClass', v)}>
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="Operational" className="text-[11px]">Operational</SelectItem>
                          <SelectItem value="Support" className="text-[11px]">Support</SelectItem>
                        </SelectContent>
                      </Select>
                    </td>
                    <td className="px-3 py-2 border-r text-[11px]">
          ...
          {/* Footer */}
          <div className="bg-white border-t px-6 py-2 flex items-center justify-between shrink-0 font-bold text-[10px] text-gray-500 uppercase tracking-widest">
